        ...
        <div class="">
            <el-card class="box-card col-sm-12">
                <div slot="header" class="clearfix">
                    <span class="h4">Договор аренды ПС (взятие/передача)</span>
                ...
                <div class="text item">
                    <el-tabs v-model="activeName">
                        ...
                        <el-tab-pane label="Прикрепленные вагоны" name="wagons">
                            <wagons-form :contract="contract"></wagons-form>
                        </el-tab-pane>
                        ...
                        <el-tab-pane label="Спецификации" name="specifications">
                            <specifications-form :contract="contract"></specifications-form>
                        </el-tab-pane>
                        <el-tab-pane label="Акты приема-передачи" name="delivery_acts">
                            <delivery-form :contract="contract" ></delivery-form>
                        </el-tab-pane>
...
import SpecificationsForm from './components/forms/SpecificationsForm.vue';
            ...
            this.$http.get(this.actions.restApi + '/' + this.$route.params.id, {})
                .then((response) => {
                    ...
                    if (response.data.hasOwnProperty('wagons')) {
                        this.contract.wagons = [];
                        var self = this;
                        response.data.wagons.every(function (el) {
                            self.contract.wagons.push({
                                id: el.id,
                                number: el.number,
                    ...
                    if (this.contract.hasOwnProperty('specifications')) {
                        var self = this;
                        let files = this.contract.specifications;
                        this.contract.specifications = [];
                        if (files !== null) {
                            files.every((el, index, arr) => {
                                self.contract.specifications_list.push({
                                    name: el.name,
                                    url: el.url,
                                    path: el.path,
                                    uid: el.uid
                                });
                                self.contract.specifications.push(el);
                                return true;
                            });
                        }
                    }
                    if (this.contract.hasOwnProperty('delivery_acts')) {
                        var self = this;
                        let files = this.contract.delivery_acts;
                        this.contract.delivery_acts = [];
                        if (files !== null) {
                            files.every((el, index, arr) => {
                                self.contract.delivery_acts_list.push({
                                    name: el.name,
                                    url: el.url,
                                    path: el.path,
                                    uid: el.uid
                                });
                                self.contract.delivery_acts.push(el);
                                return true;
                            });
                        }
                    }
