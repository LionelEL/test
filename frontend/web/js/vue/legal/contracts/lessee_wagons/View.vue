    ...
    <div>
        ...
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                <span class="h4">Договор аренды ПС (взятие/передача) № {{model.number}} от {{formatDate(model.date_start)}}</span>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <div class="">
                            ...
                            <table v-if="model.addLicenses && model.addLicenses.length" class="table details-table">
                                <thead>
                                    <tr>
                                        <th>Наименование</th>
                                        <th>Дата начала</th>
                                        <th>Внес информацию</th>
                                        <th class="action-th"><i class="fa fa-gear"></i></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="license in model.addLicenses">
                                        <td>{{ license.name }}</td>
                                        <td>{{formatDate(license.date_start)}}</td>
                                        <td>{{ license.creator }}</td>
                                        <td>
                                            <a href="#" @click="deleteLicense(license)" type="text" size="small" title="Удалить" style="margin-right:5px"><i class="el-icon-delete"></i></a>
                                            <a href="#" @click="editLicense(license.id)" type="text" size="small" title="Редактировать" style="margin-right:5px"><i class="el-icon-edit"></i></a>
                                            <a href="#" @click="viewLicense(license)" type="text" size="small" title="Просмотр"><i class="el-icon-search"></i></a>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <div v-else>
                                <em class="text-muted">нет доп. соглашений</em>
                            </div>
                        </div>
                        <div class="">
                            <h4>Прикрепленные вагоны</h4>
                            <table v-if="model.wagons && model.wagons.length" class="table details-table">
                                <thead>
                                    <tr>
                                        <th>Номер вагона</th>
                                        <th>Дата начала</th>
                                ...
                                <tbody>
                                    <tr v-for="wagon in model.wagons">
                                        <td>{{ wagon.number }}</td>
        ...
        <el-dialog  :title="'Удаление доп. соглашения ' + currentLicense.name" :visible.sync="deleteLicenseDialog" width="30%">
            <div class="text-center">
                <h4>
                    Вы действительно хотите удалить доп. соглашение
                    <em><b> {{ currentLicense.name }}</b></em>
                    от
                    <em> <b>{{ formatDate(currentLicense.date_start) }}</b>? </em>
                </h4>
            ...
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="drop(currentLicense.id)">Удалить</el-button>
                    <el-button type="default" @click="deleteLicenseDialog = false">Отмена</el-button>
            </span>
        </el-dialog>
    </div>
</template>
    ...
    data() {
        return {
            actions: {
                ...
                rest: "/api/v1/legal/licenses",
            },
            redirectUrl: '/legal/lessee-wagons',
            model: {},
        ...
        drop(id) {
            this.$http.delete(this.actions.rest + "/" + id)
                .then((response) => {
                    ...
                    this.getModel();
                    this.deleteLicenseDialog = false;
                });
        },
        formatDate(date) {
