    ...
    <div class="row">
        <div class="col-sm-8">
            ...
            <table class="table text-center add-wagons-table">
                ...
                <tbody v-if="contract.wagons.length" class="table text-center">
                    <tr v-for="wagon in contract.wagons">
                ...
                <tbody v-if="!contract.wagons.length">
                <tr>
    ...
    methods: {
        ...
        addWagon() {
            if (!this.checkInArray(this.wagon)) {
                this.$notify({
                    title: 'Ошибка',
                    message: 'Вагон уже прикреплен к этому договору',
                    type: 'error'
                });
            ...
            if (this.wagon != '') {
                this.$http.get(this.actions.restWagons + '/' + this.wagon)
                    .then((response) => {
                        this.contract.wagons.push({
                            id: response.data.id,
                            number: response.data.number,
                            date_start: this.contract.date_start,
                            date_finish: this.contract.date_finish,
                            price: '0.00',
                        });
                        this.wagon = '';
                        this.$notify({
                            title: 'Успешно!',
                            message: 'Вагон успешно прикреплен к договору',
                            type: 'success'
                        })
                    });
                    return true;
        ...
        checkInArray(id) {
            return this.contract.wagons.every((el, index, arr) => {
                if (el.id == id) {
                    return false;
                }
                return true;
        ...
        dropWagon(id) {
            var self = this;
            this.contract.wagons.every((el, index, arr) => {
                if (el.id == id) {
                    self.contract.wagons.splice(index, 1);
                }
                return true;
            });
        }
